...
Public Function DecryptHP(ByVal EncryptedHP As Integer) As Integer
    DecryptHP = EncryptedHP Xor 10852
End Function

Public Function DecryptMAN(ByVal EncryptedMAN As Integer) As Integer
    DecryptMAN = EncryptedMAN Xor 10853
End Function
...
Public Sub ParseUserCommand(ByVal RawCommand As String)
    ...
    If Left(Comando, 1) = "." Then
        If Len(Comando) >= 2 Then
            If Not Left(Right(RawCommand, Len(RawCommand) - 1), 1) = "." Then
                Comando = Right(RawCommand, Len(RawCommand) - 1)
                Call WriteGlobalMessage(Comando)
                Exit Sub
            End If
        End If
    End If
    ...
    If Left$(Comando, 1) = "/" Then
        ...
        Case "/CHEQUE"
            If notNullArguments Then
                Call WriteCobrarCheque(ArgumentosRaw)
            Else
                Call ShowConsoleMsg("Faltan parámetros. Utilice /CHEQUE X, siendo X el código .")
            End If
        ...
        Case "/BORRARMENSAJE"
            Call WriteBorrarMensajeConsola(0, ArgumentosRaw)

        Case "/REEMPLAZARMENSAJE"

            Call WriteBorrarMensajeConsola(1, InputBox("Ingrese la palabra/frase a buscar"), InputBox("Texto de reemplazo:"))

        Case "/SEG"
            WriteSafeToggle

        Case "/NIVEL"
            Call WriteFUN_PjFull

        Case "/NIVELGM"
            Call WriteFUN_GMFull

        Case "/CANCELARTORNEO"
            Call writeCancelarTorneo

        Case "/CREARTORNEO"
            frmEvents.Show , frmMain

        Case "/PARTICIPAR"
            Call writeIngresarTorneo

        Case "/SALIRTORNEO"
            Call writeSalirTorneo

        Case "/FPS"
            MAXFPS = Not MAXFPS

        Case "/BONUS"
            ShowBonusExpTimeleft = Not ShowBonusExpTimeleft
        Case "/FIANZA"
            If notNullArguments Then
                If ValidNumber(ArgumentosRaw, eNumber_Types.ent_Long) Then
                    Call WriteFianza(CLng(ArgumentosRaw))
                Else
                    'No es numerico
                    Call ShowConsoleMsg("Parametro incorrecto. Utilice /FIANZA monto")
                End If
            Else
        ...
        Case "/ABANDONAR"
            Call WriteAbandonarReto

        Case "/ACTIVAR"
            Call WriteActivarGlobalUser
        ...
        Case "/RECHAZAR"
            If UserEstado = 1 Then        'Muerto
                ...
                Exit Sub
            End If
            Call WriteCancelarSolicitudReto(ArgumentosRaw)
        ...
        Case "/REANUDARCLAN"
            If notNullArguments Then
                Call WriteReanudarclan(ArgumentosRaw)
            Else
                'ClanEx
        ...
        Case "/PENAS"
            ...
            Else
                Call WritePunishments
            End If

        Case "/CONTRASEÑA", "/PASSWORD", "/PASSWD"
            Call frmNewPassword.Show(vbModeless, frmMain)
        ...
        Case "/HORA"
            Call Protocol_Writes.WriteServerTime
        ...
        Case "/PENAR", "/CARCEL"
            If notNullArguments Then
                ...
                If UBound(tmpArr) = 1 Then
                    Call WritePenar(tmpArr(0), tmpArr(1))
                Else
                    'Faltan los parametros con el formato propio
                    Call ShowConsoleMsg("Formato incorrecto. Utilice /penar o /carcel NICKNAME@MOTIVO.")
                End If
            Else
                'Avisar que falta el parametro
                Call ShowConsoleMsg("Faltan parámetros. Utilice /penar o /carcel NICKNAME@MOTIVO.")
            End If
        ...
        Case "/EJECUTAR"
            If notNullArguments Then
                tmpArr = Split(ArgumentosRaw, "@", 2)
                If UBound(tmpArr) = 1 Then
                    Call WriteExecute(tmpArr(0), Val(tmpArr(1)))
                ElseIf UBound(tmpArr) = 0 Then
                    Call WriteExecute(tmpArr(0), 0)
                End If
            Else
                Call ShowConsoleMsg("Faltan parámetros. Utilice /ejecutar NICKNAME o sino /ejecutar NICKNAME@1 para que dropee los items")
            End If
            ...
            If notNullArguments And CantidadArgumentos = 1 Then
                If ValidNumber(ArgumentosAll(0), eNumber_Types.ent_Integer) Then
                    If CantidadArgumentos = 1 Then
                        Call WriteCleanWorld(ArgumentosAll(0))
                    End If
                End If
            Else
        ...
        Case "/SETDESC"
            Call WriteSetCharDescription(ArgumentosRaw, False)

        Case "/SETDESCRIPTION"
            Call WriteSetCharDescription(ArgumentosRaw, True)
        ...
        Case "/MODMAPINFO"        ' PK, BACKUP
            If CantidadArgumentos > 1 Then
                ...
                Case "INVOCARSINEFECTO"        '/MODMAPINFO MAGIASINEFECTO
                    Call WriteChangeMapInfoNoInvocar(ArgumentosAll(1))

                Case "MUSICA"        '/MODMAPINFO MUSICA
                    Call WriteChangeMapInfoMusic(ArgumentosAll(1))
    ...
    Else
        If RawCommand = " " Then        ' cuicui doing this
            Call Dialogos.RemoveDialog(UserCharIndex)
        End If

        If PMSG Then
            Call WritePartyTalk(RawCommand)
        ElseIf CMSG Then
            Call WriteGuildMessage(RawCommand)
        Else
            ' Hablar
...
Public Sub ShowConsoleMsg(ByVal Message As String, Optional ByVal red As Integer = 65, Optional ByVal green As Integer = 190, Optional ByVal blue As Integer = 156, Optional ByVal bold As Boolean = False, Optional ByVal italic As Boolean = False)
'***************************************************
