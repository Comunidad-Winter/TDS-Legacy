...
Public Function LoadJPG( _
    ...
    If lR = IJL_OK Then
        ...
        ' Write the filename to the jcprops.JkGFile member:
...
'Medio desprolijo donde pongo la pic, pero es lo que hay por ahora
    ...
    frmScreenshots.Picture1.Width = 12030
    frmScreenshots.Picture1.Height = 9030
    ...
    dirFile = IIf(Autofragshooter, "\Fotos\FragShooter", "\Fotos")

    If Not FileExist(App.Path & dirFile, vbDirectory) Then MkDir (App.Path & dirFile)

    File = App.Path & dirFile & "\TDSL_foto_" & format(Now, "yyyy_mm_dd_hh_mm_ss") & ".jpg"
    ...
    AddtoRichTextBox frmMain.RecTxt, "Guardado: " & File, 0, 200, 200, False, False, True
    Exit Sub
