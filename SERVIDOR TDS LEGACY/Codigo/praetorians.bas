...
Sub PRCAZA_AI(ByVal npcind As Integer)
    ...
    For X = NPCPosX + (azar * 8) To NPCPosX + (azar * -8) Step -azar
        For Y = NPCPosY + (azar2 * 7) To NPCPosY + (azar2 * -7) Step -azar2
            ...
            If (PJEnInd > 0) And NpcIntervaloGolpe(npcind) Then
    ...
    Case 1        ''nearest target
        If (NpcIntervaloGolpe(npcind)) Then
            Call NpcLanzaSpellSobreUser(npcind, BestTarget, Npclist(npcind).Spells(1).SpellID)
        End If
    ...
    Else
        ...
        If (NpcIntervaloGolpe(npcind)) Then Call VolverAlCentro(npcind)
...
Sub PRMAGO_AI(ByVal npcind As Integer)
    ...
    Case 2        ''apocalipsis Rahma Nañarak O'al
        Call SendData(SendTarget.ToNPCArea, npcind, PrepareMessageChatOverHead(Hechizos(Npclist(npcind).Spells(DAT_APOCALIPSIS).SpellID).PalabrasMagicas, Npclist(npcind).Char.CharIndex, vbCyan))
        Call NpcLanzaSpellSobreUser2(npcind, BestTarget, Npclist(npcind).Spells(DAT_APOCALIPSIS).SpellID)        ''SPELL 1 de Mago: Apocalipsis
    Case 3
    ...
    If Not (quehacer = 3) Then        ''si no ta matandose
        ...
        If (BestTarget = 0) And (NpcIntervaloGolpe(npcind)) Then Call VolverAlCentro(npcind)
...
Sub PRREY_AI(ByVal npcind As Integer)
    ...
    For X = NPCPosX - 8 To NPCPosX + 8
        For Y = NPCPosY - 7 To NPCPosY + 7
            ...
            PJEnInd = MapData(NPCPosM, X, Y).UserIndex
            If (NpcIntervaloGolpe(npcind)) Then        ''saltea el analisis si no puede atacar para evitar cuentas
                If (NPCAlInd > 0) Then
    ...
    If Not hayPretorianos Then
            ...
            For headingloop = eHeading.NORTH To eHeading.WEST
                ...
                If InMapBounds(nPos.map, nPos.X, nPos.Y) Then
                    ...
                    If UI > 0 Then
                        ...
                        NpcIntervaloGolpe (npcind)        ''this is NOT a bug!!
                        '----------------------------------------------------
        ...
        Else        ''no hay targets cerca
            Call VolverAlCentro(npcind)
            If (Npclist(npcind).Stats.MinHP < Npclist(npcind).Stats.MaxHP) And (NpcIntervaloGolpe(npcind)) Then
...
Sub PRCLER_AI(ByVal npcind As Integer)
    ...
    Case 1        '' paralizar enemigo PJ
        Call SendData(SendTarget.ToNPCArea, npcind, PrepareMessageChatOverHead(Hechizos(Npclist(npcind).Spells(DAT_PARALIZARPJ).SpellID).PalabrasMagicas, Npclist(npcind).Char.CharIndex, vbCyan))
        Call NpcLanzaSpellSobreUser(npcind, BestTarget, Npclist(npcind).Spells(DAT_PARALIZARPJ).SpellID)        ''SPELL 1 de Clerigo es PARALIZAR
        Exit Sub
    Case 2        '' ataque a usuarios (invisibles tambien)
        Call SendData(SendTarget.ToNPCArea, npcind, PrepareMessageChatOverHead(Hechizos(Npclist(npcind).Spells(DAT_TORMENTAAVANZADA).SpellID).PalabrasMagicas, Npclist(npcind).Char.CharIndex, vbCyan))
        Call NpcLanzaSpellSobreUser2(npcind, BestTarget, Npclist(npcind).Spells(DAT_TORMENTAAVANZADA).SpellID)        ''SPELL 2 de Clerigo es Vax On Tar avanzado
        Exit Sub
...
Sub NPCRemueveVenenoNPC(ByVal npcind As Integer, ByVal NPCAlInd As Integer, ByVal indice As Integer)
    ...
    indireccion = Npclist(npcind).Spells(indice).SpellID
...
Sub NPCparalizaNPC(ByVal paralizador As Integer, ByVal Paralizado As Integer, ByVal indice)
    ...
    indireccion = Npclist(paralizador).Spells(indice).SpellID
    '' Envia las palabras magicas, fx y wav del indice-esimo hechizo del npc-hostiles.dat
...
Sub NPCcuraNPC(ByVal curador As Integer, ByVal curado As Integer, ByVal indice As Integer)
    ...
    indireccion = Npclist(curador).Spells(indice).SpellID
    '' Envia las palabras magicas, fx y wav del indice-esimo hechizo del npc-hostiles.dat
...
Sub NPCLanzaCegueraPJ(ByVal npcind As Integer, ByVal PJEnInd As Integer, ByVal indice As Integer)
    ...
    If Npclist(npcind).Name = "Rey Pretoriano" Then
        Call WriteMensajes(PJEnInd, Mensaje_422, FontTypeNames.FONTTYPE_FIGHT)
        Call WriteMensajes(PJEnInd, Mensaje_423, FontTypeNames.FONTTYPE_VENENO)
    End If
...
Sub NPCLanzaEstupidezPJ(ByVal npcind As Integer, ByVal PJEnInd As Integer, ByVal indice As Integer)
    ...
    'bardea si es el rey
    If Npclist(npcind).Name = "Rey Pretoriano" Then
        Call WriteMensajes(PJEnInd, Mensaje_424, FontTypeNames.FONTTYPE_FIGHT)
    End If
...
Sub NPCRemueveInvisibilidad(ByVal npcind As Integer, ByVal PJEnInd As Integer, ByVal indice As Integer)
    ...
    If UserList(PJEnInd).flags.oculto = 1 Then
        ...
        Call UsUaRiOs.SetInvisible(PJEnInd, UserList(PJEnInd).Char.CharIndex, UserList(PJEnInd).flags.invisible = 1, UserList(PJEnInd).flags.oculto = 1)
        'Call SendData(SendTarget.ToPCArea, PJEnInd, PrepareMessageSetInvisible(UserList(PJEnInd).Char.CharIndex, False))
        Call WriteMensajes(PJEnInd, Mensaje_425, FontTypeNames.FONTTYPE_VENENO)
    Else
        'sino, solo lo "iniciamos" en la sacada de invisibilidad.
        Call WriteMensajes(PJEnInd, Mensaje_23, FontTypeNames.FONTTYPE_VENENO)
...
Sub NpcLanzaSpellSobreUser2(ByVal NpcIndex As Integer, ByVal UserIndex As Integer, ByVal Spell As Integer)
    ...
    If NpcIntervaloGolpe(NpcIndex) = 0 Then Exit Sub
    ...
    ElseIf Hechizos(Spell).SubeHP = 2 Then
        ...
        If Not EsGM(UserIndex) Then UserList(UserIndex).Stats.MinHP = UserList(UserIndex).Stats.MinHP - Daño
        ...
        For X = 8 To 95
            For Y = 8 To 95
                ...
                If PJInd > 0 Then
                    ...
                    ''efectiviza el danio
                    If Not EsGM(PJInd) Then UserList(PJInd).Stats.MinHP = UserList(PJInd).Stats.MinHP - danioI
    ...
    If (NPCx > USRx) Then
        ...
        Else        ''x completitud, esta en la misma Y
            ...
            Else
                ...
                If NpcIntervaloGolpe(npcorig) And (RandomNumber(1, 100) > 95) Then
