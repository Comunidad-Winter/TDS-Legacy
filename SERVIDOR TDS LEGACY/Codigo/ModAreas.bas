        ...
        For X = MinX To MaxX
            For Y = MinY To MaxY
                    ...
                    If UserIndex <> TempInt Then
                        ...
                        If Not (UserList(TempInt).flags.AdminInvisible = 1) Then
                            ...
                            If UserList(TempInt).flags.invisible Or UserList(TempInt).flags.oculto Then
                                If Not EsGM(UserIndex) Then
                                    Call WriteSetInvisible(UserIndex, UserList(TempInt).Char.CharIndex, UserList(TempInt).flags.invisible = 1, UserList(TempInt).flags.oculto = 1)
                                End If
                            End If
                        End If
                        ...
                        If Not (.flags.AdminInvisible = 1) Then
                            ...
                            If .flags.invisible Or .flags.oculto Then
                                If Not EsGM(UserIndex) Then
                                    Call WriteSetInvisible(TempInt, .Char.CharIndex, .flags.invisible = 1, .flags.oculto = 1)
                                End If
                            End If
                        End If
                ...
                If MapData(map, X, Y).ObjInfo.ObjIndex Then
                    ...
                    If ObjData(TempInt).OBJType <> eOBJType.otArboles Then
                ...
                '<<< Bots >>>
                If MapData(map, X, Y).BotIndex Then
                    TempInt = MapData(map, X, Y).BotIndex

                    If IA_Bot(TempInt).Summoned Then
                        Call IA_EnviarChar(UserIndex, TempInt)
                    End If
                End If
...
'**************************************************************
    ...
    'Search for the user
1   For LoopC = 1 To ConnGroups(map).CountEntrys
2       If ConnGroups(map).UserEntrys(LoopC) = UserIndex Then Exit For
3   Next LoopC

    'Char not found
4   If LoopC > ConnGroups(map).CountEntrys Then Exit Sub

    'Remove from old map
5   ConnGroups(map).CountEntrys = ConnGroups(map).CountEntrys - 1
6   TempVal = ConnGroups(map).CountEntrys

    'Move list back
7   For LoopC = LoopC To TempVal
8       ConnGroups(map).UserEntrys(LoopC) = ConnGroups(map).UserEntrys(LoopC + 1)
9   Next LoopC

10  If TempVal > ConnGroups(map).OptValue Then        'Nescesito Redim?
11      ReDim Preserve ConnGroups(map).UserEntrys(1 To TempVal) As Long
12  End If
End Sub
