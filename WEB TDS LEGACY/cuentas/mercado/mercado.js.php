	...
	tabla= document.getElementById('contenido');
		...
		try {
			...
			if (pjsOfrecidos != null) {
				...
				if (comentario.indexOf ('http://', 0) != -1 || comentario.indexOf ('https://', 0) != -1 || comentario.indexOf ('www', 0) != -1 || comentario.indexOf ('.com', 0) != -1  || comentario.indexOf ('.php', 0) != -1) {
					alert ('No se pueden poner links en el comentario');
					return false;
				}
				
			} else if (pjSelect!= null) { 
...
function pasar() {
	...
	if (obj.selectedIndex==-1) return;
		...
		alert('Para poder publicar más de un personaje en la misma publicacion tu cuenta debe ser Premium.');
		return false;
...
function actualizarImpuesto() {
    ...
    if (modo[1].checked) {
        if (infoPrecio!=undefined) {
            var comienzo = txt.indexOf('(');
            var fin = txt.lastIndexOf(')');
            
            nivel = parseInt(txt.substr(comienzo+1, fin-comienzo-1));
            var impuesto = nivel * 1000;
            
            infoPrecio.innerHTML = "<strong>IMPORTANTE: </strong>Para publicar este personaje deber&aacute;s <strong>pagar "+impuesto+" monedas de oro</strong>. Estas deben estar en tu billetera y ser&aacute;n retiradas cuando se haga la publicaci&oacute;n.";
