...
.buy-button:hover {
    ...
    <section class="container">
        ...
        <label for="pjSeleccionado">Escriba por favor qué personaje obtendrá éste beneficio:<br><small>(Debe ser de tu propiedad o no debe estar en ninguna Cuenta Premium)</small><br></label><br>
        <input style="width: 50%" type="text" id="pjSeleccionado" name="pjSeleccionado" placeholder="Escribe el nombre del personaje">
        <br><br>
        <div class="products">
    ...
    function loadProducts() {
        ...
        xhr.open('GET', 'cuentas/tienda_backend.php', true);
        xhr.onload = function() {
    ...
    function displayProduct(producto) {
        ...
        if (producto.adhonorem==1) {
            div.innerHTML = `
                <img src="imagenes/${producto.imagen}.png" alt="${producto.nombre}">
                <div class="product-info">
                    <h3>${producto.nombre}</h3>
                    <p class="description">${producto.descripcion}</p><br>                    
                    <input type="number" id="adhonorem-${producto.id}" placeholder="Monto a donar" >
                    <button class="add-to-cart" data-id="${producto.id}">Donar</button>
                </div>
            `;
        ...
        productsContainer.appendChild(div);
                ...
                <script>
                    document.addEventListener('DOMContentLoaded', function() {
                        ...
                        const productsContainer = document.querySelector('.products');
                        const personajesSelect = document.getElementById('pjSeleccionado');
                        ...
                        productsContainer.addEventListener('click', function(event) {
                            if (event.target.classList.contains('add-to-cart')) {
                                ...
                                const selectedCharacter = personajesSelect.value;
                                
                                const adhonoremElement = document.getElementById('adhonorem-' + productId);
                                let adhonorem = 0;
                                if (adhonoremElement) {
                                    adhonorem = adhonoremElement.value;
                                }

                                console.log('Procesando compra del producto con ID:', productId);
                                
                                 fetch('cuentas/tienda_backend.php', {
                                     method: 'POST',
                                     body: JSON.stringify({ productId: productId, pjSeleccionado: selectedCharacter, adhonorem:adhonorem }),
                                     headers: {
