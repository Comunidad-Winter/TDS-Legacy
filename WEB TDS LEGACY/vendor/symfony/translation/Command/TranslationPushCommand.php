...
EOF
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if ($deleteMissing) {
            ...
            $io->success(sprintf('Missing translations on "%s" has been DEL (for "%s" locale(s), and "%s" domain(s)).', parse_url($provider, \PHP_URL_SCHEME), implode(', ', $locales), implode(', ', $domains)));

            // Read provider translations again, after missing translations deletion,
            // to avoid push freshly DEL translations.
            $providerTranslations = $provider->read($domains, $locales);
        }
