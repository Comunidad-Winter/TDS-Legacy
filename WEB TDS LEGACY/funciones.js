...
return xmlhttp;}
function cargarComent(donde,id){var ajax2=nuevoAjax();ajax2.open("GET","comentarios/control.php?id="+parseInt(id));ajax2.onreadystatechange=function(){if(ajax2.readyState==4){document.getElementById(donde).innerHTML=ajax2.responseText;}}
ajax2.send(null);}
...
if((formulario.mail.value.indexOf('@',0)==-1)||(formulario.mail.value.indexOf('.',0)==-1)){alert("Escriba una direccion de correo valida en el campo \"Email\".");formulario.mail.focus()
return(false);}
return(true);}
function borrar(campo){campo.value='';}
function verimagen_noticias(url){var theTop=30;var imagen=document.getElementById('imagengrande');imagen.innerHTML='<img src="'+url+'"  title="Click aqui para cerrar la imagen" border="1"/>';imagen.style.display='block';if(window.innerHeight){pos=window.pageYOffset}else if(document.documentElement&&document.documentElement.scrollTop){pos=document.documentElement.scrollTop}else if(document.body){pos=document.body.scrollTop}
if(pos<theTop)
pos=theTop;else
