<?php
	...
	if(count($errors) === 0) {
		...
		if(isset($_POST['csrf_token']) && validateToken($_POST['csrf_token'])) {
			...
			$C = connect();
			if($C) {
				// Get user, hash, & timestamp from password_requests
				...
				if($res && $res->num_rows === 1) {
					...
					if(password_verify(urlSafeDecode($_POST['hash']), $request['hash'])) {
						...
						if($request['timestamp'] >= time() - PASSWORD_RESET_REQUEST_EXPIRY_TIME) {
							// update password
							$hash = $_POST['password'];//password_hash($_POST['password'], PASSWORD_DEFAULT);
							if(sqlUpdate($C, 'UPDATE user SET password=? WHERE id=?', 'si', $hash, $request['user'])) {
								// delete all password reset password_requests for user
